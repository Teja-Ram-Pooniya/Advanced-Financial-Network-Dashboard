...
mu = st.sidebar.slider("Drift ($\mu$)", -0.5, 0.5, 0.05)
sigma = st.sidebar.slider("Continuous Vol ($\sigma$)", 0.05, 0.8, 0.2)
lam = st.sidebar.slider("Poisson Jump Intensity ($\lambda$)", 0.1, 10.0, 4.0, help="Expected number of jumps per year")
...
with tab2:
    ...
    fig = px.line(sim_df, x="Trading Day", y=["Merton Jump Diffusion Path", "Standard Geometric Brownian Motion"],
                  title="Asset Price Simulation under $\mathbb{Q}$-Measure")
